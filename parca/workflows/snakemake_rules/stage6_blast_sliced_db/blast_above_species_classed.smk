...
checkpoint prepare_blast_input:
    ...
    output: 
        blast_infiles=temp(directory("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage5/sliceblastin")),
        read_count=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/stats_{sample_type}_{nucleotide}/stage5/count_printedfiles_assembledreadlengths.txt")
    params: 
        chunk_size=6000
...
rule blast_slices:
    ...
    output:
        blast_out=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage6/sliceblastout/{gi_slice}__{sliceiter}")
    params:
...
rule merge_slice_blast_result:
    ...
    output: 
        best_blast=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage6/best_blast.txt")
...
rule taxonomic_lineage_best_blast:
    ...
    output:
        tax_id_lineage=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage6/best_blast_tax_id_lineage.txt")
...
rule reformat_blast_taxids:
    ...
    output: 
        species_and_above=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage6/best_blast_species_and_above.txt"),
        count_reads_tax_ids=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/stats_{sample_type}_{nucleotide}/stage6/count_reads_taxid_SubsetBLAST.txt")
    conda: "../../conda/R_env.yaml" #config['conda_environment'] 
    params:
...
rule merge_kmer_classed_and_blast_classed:
    ...
    output: 
        kmer_blast=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/{sample_type}_{nucleotide}/stage6/kmer_species_subsetblast_classed.txt"),
        count_kmer_blast=temp("{outdir}/{start_date}_{run_id}/snakemake_results_{sample}/stats_{sample_type}_{nucleotide}/stage6/count_kmer_SubsetBLAST.txt")
    shell: 
        """
